...
SC_MODULE(LSTM_neural) {
    ...
    float xf_weight_matrix[10], xi_weight_matrix[10];
    float xo_weight_matrix[10], xc_weight_matrix[10];
...
void LSTM_neural::lstm_process() {
    while (true) {
        if (rst.read()) {
            ...
            for (int i = 0; i < 10; i++) {
                xf_mul[i] = x_val * xf_weight_matrix[i];
                xi_mul[i] = x_val * xi_weight_matrix[i];
                xo_mul[i] = x_val * xo_weight_matrix[i];
                xc_mul[i] = x_val * xc_weight_matrix[i];
                wait();
            }
...
float LSTM_neural::exp_approx(float x) {
...
float LSTM_neural::sigmoid(float x) {
    return 1.0f / (1.0f + exp_approx(-x));
}

float LSTM_neural::tanh(float x) {
